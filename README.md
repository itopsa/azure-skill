...
A token-efficient, CLI-driven, and progressive-disclosure Azure management skill for AI coding assistants (Antigravity, Gemini CLI, Claude, and GitHub Copilot) with full cross-platform support for Windows (PowerShell), macOS, Linux, Azure DevOps (ADO), and JetBrains TeamCity.
...
* [IaC & Deployments](./.agents/skills/azure/references/iac-deployments.md) — Bicep dry-runs (`what-if`) and ARM templates.
...
* [TeamCity Reference](./.agents/skills/azure/references/teamcity.md) — TeamCity REST API, build triggers, artifact downloads, and service messages.
...
1. **Native CLI & IaC Direct Execution:** Uses the Azure CLI (`az`), Bicep, Azure DevOps CLI (`az devops`), and TeamCity REST/CLI directly for deterministic operations.
...
│               ├── iac-deployments.md   # Bicep, ARM what-if workflows
...
│               └── teamcity.md          # TeamCity build triggers, logs, artifacts & REST
├── .github/
...
Drop the `.agents/skills/azure` directory into your project root or install globally in `~/.gemini/config/skills/azure`. The agent will automatically detect Azure/CI/CD tasks and activate the skill.
...
* **On-Demand in Chat:** Mention specific runbooks using `@workspace #file:.agents/skills/azure/references/observability-logs.md` or `#file:.agents/skills/azure/references/teamcity.md`.
